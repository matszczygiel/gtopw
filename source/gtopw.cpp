        ...
        for(usint j=0; j<=i; j++) {
            ...
            for(usint k=0; k<=i; k++) {
                ...
                for(usint l=0; l<lmax; l++) {
                        ...
                        for(usint k2=0; k2<basis[j].alphaA.size(); k2++ ) {
                            ...
                            for(usint k3=0; k3<basis[k].alphaA.size(); k3++ ) {
                                ...
                                for(usint k4=0; k4<basis[l].alphaA.size(); k4++ ) {
                                    ...
                                    Qx = ak * Cx + al * Dx; Qx = Qx / akl;
                                    Qy = ak * Cy + al * Dy; Qy = Qy / akl;
                                    Qz = ak * Cz + al * Dz; Qz = Qz / akl;
                        ...
                        for(mj=0; mj<shgj; mj++) {
                            ...
                            for(mk=0; mk<shgk; mk++) {
                                ...
                                for(ml=0; ml<shgl; ml++) {
                                    ...
                                    full_trans_crt.v[mi][mj][mk][ml] = full_trans_crt.v[mi][mj][mk][ml]
                                                                 / xyz_norm[li][mi] / xyz_norm[lj][mj]
                                                                 / xyz_norm[lk][mk] / xyz_norm[ll][ml];
                    ...
                    /* write down */
                    full_trans_crt.load_pos_mx( posimx_crt, posjmx_crt, 
...
void Init() {
    ...
    belt[0] = 1;
    for(int i=1; i< 500; i++) belt[i] = -belt[i-1];
    
    fact[0] = 1.0;
    fact[1] = 1.0;
    for(int i=2; i<=160; i++) fact[i] = fact[i-1] * i;
    ...
    dfact[0] = 1.0;
    dfact[1] = 1.0;
    for(int i=1; i<200; i++) dfact[i+1] = dfact[i] * ( 2*i + 1 );
    
    for(int i=0; i<=32; i++) for(int j=0; j<=i; j++)
        omega[i][j] = sqrt( ( 2.0*i + 1 ) * fact[i-j] / ( 2.0 * fact[i+j] ) );
            
    int mm,pos,lx,ly,lz;
    for(int l=0; l<=bas_lmax; l++) for(int m=-l; m<=l; m++) {
        mm = m + l;
        for(int i=0; i<=l; i++) for(int j=0; j<=l-i; j++) {
            pos = i * (2 * l + 3 - i) / 2 + j;
            lx  = i;
            ly  = j;
            lz  = l - i - j;
