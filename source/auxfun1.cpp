...
void TransToSpher1D(cdouble* shl_crt,
	...
	for (mi = 0; mi < shi; mi++) {
		...
		for (mig = 0; mig < shgi; mig++) {
			tmp += shl_crt[mig] * clmr[li][mi][mig];
		};
		...
		for (mj = 0; mj < shj; mj++) {
			...
			for (mig = 0; mig < shgi; mig++) {
				...
				for (mjg = 0; mjg < shgj; mjg++) {
					tmp += shl_crt[mig * shgj + mjg] * clmr[li][mi][mig] * clmr[lj][mj][mjg];
					jb++;
					if (jb > lj - ib) {
						jb = 0;
						ib++;
...
void CalcRNucA(RInts1E<cdouble>& R) {
	...
	if (bigT.real() > 0.0) {
		...
		ex1 = (R.Px - R.Cx) * (R.Px - R.Cx) + (R.Py - R.Cy) * (R.Py - R.Cy) + (R.Pz - R.Cz) * (R.Pz - R.Cz);
		ex1 = exp(-R.aP * ex1);
	...
	if (R.lsum > 0) {
		for (int j = 0; j < R.lsum; j++)
			R.rtuvj[0][0][1][j] = faz * R.rtuvj[0][0][0][j + 1] + fiz * R.rtuvj[0][0][0][j];
	};

	for (int v = 1; v < R.lsum; v++)
		for (int j = 0; j < R.lsum - v; j++)
			R.rtuvj[0][0][v + 1][j] = faz * R.rtuvj[0][0][v][j + 1] + fiz * R.rtuvj[0][0][v][j] + (double)v * R.rtuvj[0][0][v - 1][j + 1];

	/* R_{0uvj}: grow u at cost of j, constant v */
	if (R.lsum > 0) {
		for (int v = 0; v <= R.lsum; v++)
			for (int j = 0; j < R.lsum - v; j++)
				R.rtuvj[0][1][v][j] = fay * R.rtuvj[0][0][v][j + 1] + fiy * R.rtuvj[0][0][v][j];
	};

	for (int u = 1; u < R.lsum; u++)
		for (int v = 0; v <= R.lsum; v++)
			for (int j = 0; j < R.lsum - v - u; j++)
				R.rtuvj[0][u + 1][v][j] = fay * R.rtuvj[0][u][v][j + 1] + fiy * R.rtuvj[0][u][v][j] + (double)u * R.rtuvj[0][u - 1][v][j + 1];

	/* R_{tuvj}: grow t at cost of j, constant u and v */
	if (R.lsum > 0) {
		for (int u = 0; u <= R.lsum; u++)
			for (int v = 0; v <= R.lsum; v++)
				for (int j = 0; j < R.lsum - v - u; j++)
					R.rtuvj[1][u][v][j] = fax * R.rtuvj[0][u][v][j + 1] + fix * R.rtuvj[0][u][v][j];
	};

	for (int t = 1; t < R.lsum; t++)
		for (int u = 0; u <= R.lsum; u++)
			for (int v = 0; v <= R.lsum; v++)
				for (int j = 0; j < R.lsum - v - u - t; j++)
					R.rtuvj[t + 1][u][v][j] = fax * R.rtuvj[t][u][v][j + 1] + fix * R.rtuvj[t][u][v][j] + (double)t * R.rtuvj[t - 1][u][v][j + 1];
	R.strip();
	/* */
};
	...
	if (m > 0) {
		...
		for (int a = max(bs, 0); a <= lima; a++) {
			...
			for (int b = lim1b; b <= lim2b; b++) {
				...
				val += binom[bs][b] * binom[am][twob] * belt[twob / 2] * faci;
			};
		};
			...
			if (rez + imz <= 36.0 && rez <= 20.0) {  // Luke's rational appr.
				...
				for (int m = 0; m <= v.nmax; m++) {
					...
					for (int n = 3; n < nfmx; n++) {
						...
						AnR[n] = (1.0 + rez * F1) * AnR[n - 1] - F1 * imz * AnI[n - 1] + (x2y2 * F2 - F1 * rez) * AnR[n - 2] - (xy * F2 - F1 * imz) * AnI[n - 2] + F3 * rez * x3y * AnR[n - 3] - F3 * imz * y3x * AnI[n - 3];

						AnI[n] = (1.0 + rez * F1) * AnI[n - 1] + F1 * imz * AnR[n - 1] + (x2y2 * F2 - F1 * rez) * AnI[n - 2] + (xy * F2 - F1 * imz) * AnR[n - 2] + F3 * rez * x3y * AnI[n - 3] + F3 * imz * y3x * AnR[n - 3];
					};

					den = twom * (BnR[nfmx1] * BnR[nfmx1] + BnI[nfmx1] * BnI[nfmx1]);
					v.fn[m] = (AnR[nfmx1] * BnR[nfmx1] + AnI[nfmx1] * BnI[nfmx1]) / den + I * (AnI[nfmx1] * BnR[nfmx1] - AnR[nfmx1] * BnI[nfmx1]) / den;
				};
			...
			if (abs(rez) + abs(imz) <= 51.0 &&
				...
				for (int m = 0; m <= v.nmax; m++) {
					...
					for (int n = 2; n < nfmx; n++) {
						...
						F2 = 8.0 * (n - 1) * (n + n + twom - 2) / (tmp + 2) / tmp / tmp / (tmp - 2);

						AnR[n] = (1.0 + F1 * rez) * AnR[n - 1] - F1 * imz * AnI[n - 1] + F2 * (x2y2 * AnR[n - 2] - xy * AnI[n - 2]);
						AnI[n] = (1.0 + F1 * rez) * AnI[n - 1] + F1 * imz * AnR[n - 1] + F2 * (x2y2 * AnI[n - 2] + xy * AnR[n - 2]);

						BnR[n] = (1.0 + F1 * rez) * BnR[n - 1] - F1 * imz * BnI[n - 1] + F2 * (x2y2 * BnR[n - 2] - xy * BnI[n - 2]);
						BnI[n] = (1.0 + F1 * rez) * BnI[n - 1] + F1 * imz * BnR[n - 1] + F2 * (x2y2 * BnI[n - 2] + xy * BnR[n - 2]);
					};
